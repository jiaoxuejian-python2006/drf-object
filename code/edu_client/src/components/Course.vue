            ...
            <div class="course-list">
                <div class="course-item" v-for="course in course_list">
                    ...
                    <div class="course-info">
                        <h3><router-link :to="`/detail/`+course.id">{{ course.name }} </router-link><span><img src="/static/image/avatar1.svg" alt="">{{
                                course.students
                            }}人已加入学习</span></h3>
                        <p class="teather-info">huxz 百知教育教学总监 <span>共{{ course.lessons }}课时/{{
                        ...
                        <ul class="lesson-list">
                            <li v-for="(lesson, key) in course.lesson_list"><span class="lesson-title">{{ key + 1 }} | 第{{
                                    key + 1
                                }}节：{{ lesson.name }}</span> <span class="free" v-if="lesson.free_trail">免费</span></li>
                        </ul>
                        <div class="pay-box">
...
export default {
    ...
    // 检测页面中分类id的值的变化，一旦变化，则根据当前点击的id获取对用的课程
    watch: {
        category() {
            // 重新按照当前的id发送请求
            this.filters.page=1
    ...
    methods: {
        ...
        get_all_course() {
            ...
            this.$axios.get("http://127.0.0.1:8000/course/list/", {params: filters}
            ).then(res => {
